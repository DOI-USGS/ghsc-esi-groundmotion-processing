...
- "metric_dict" can be inspected by loooking at the prior_step/output attributes, e.g.
  ...
  And the result of the previous step can be accessed with
    ...
    metric_dict["test1"].prior_step.output
...
from gmprocess.metrics import combine  # noqa
from gmprocess.metrics import reduce  # noqa
from gmprocess.metrics import rotate  # noqa
from gmprocess.metrics import transform  # noqa
    ...
    def calculate(self):
        ...
        for metric, metric_steps in self.steps.items():
            ...
            for metric_step in metric_steps:
                ...
                parameters = step_class.get_parameters(self.config)
                parameter_list = self._flatten_params(parameters)
                next_step = []
                for prior_step in metric_results:
                    for params in parameter_list:
                        next_step.append(step_class(prior_step, params))
