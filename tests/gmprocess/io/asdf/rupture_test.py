...
def test_rupture_quad():
    # this rupture file contains multiple polygons that each have 5 vertices
    # (with the first and last being the same)
...
def test_rupture_complex():
    # this rupture file contains seven polygons, two of which have more than 5 vertices
    # (they have 7 vertices)
    file_loc = Path(__file__)
    test_data_loc = file_loc.parent.parent.parent.parent / "data" / "asdf" / "rupture"
...
def test_get_rupture_quad():
    ...
    assert (
        description == "Source: Bradley et al (2017) Ground motion Observations "
        "from the 14 November 1 2016 Mw7.8 Kaikoura, New Zealand earthquake"
    )
    assert (
        reference == "Source: Bradley et al (2017) Ground motion Observations "
        "from the 14 November 1 2016 Mw7.8 Kaikoura, New Zealand earthquake"
    )
...
def test_get_rupture_complex():
    ...
    assert (
        description == "SOURCE: Barka, A., H. S. Akyüz, E. Altunel, G. Sunal, Z. "
        "Çakir, A. Dikbas, B. Yerli, R. Armijo, B. Meyer, J. B. d. Chabalier, T. "
        "Rockwell, J. R. Dolan, R. Hartleb, T. Dawson, S. Christofferson, A. Tucker, "
        "T. Fumal, R. Langridge, H. Stenner, W. Lettis, J. Bachhuber, and W. Page "
        "(2002). The Surface Rupture and Slip Distribution of the 17 August 1999 "
        "Izmit Earthquake (M 7.4), North Anatolian Fault, Bull. Seism. Soc. Am. 92, "
        "43-60."
    )
    assert (
        reference == "SOURCE: Barka, A., H. S. Akyüz, E. Altunel, G. Sunal, Z. "
        "Çakir, A. Dikbas, B. Yerli, R. Armijo, B. Meyer, J. B. d. Chabalier, T. "
        "Rockwell, J. R. Dolan, R. Hartleb, T. Dawson, S. Christofferson, A. Tucker, "
        "T. Fumal, R. Langridge, H. Stenner, W. Lettis, J. Bachhuber, and W. Page "
        "(2002). The Surface Rupture and Slip Distribution of the 17 August 1999 "
        "Izmit Earthquake (M 7.4), North Anatolian Fault, Bull. Seism. Soc. Am. "
        "92, 43-60."
    )
