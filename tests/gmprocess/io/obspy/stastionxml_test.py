...
def test():
    ...
    data_files, event = read_data_dir("station_xml_epochs", "nc73631381", "*.mseed")
    ...
    data_files, event = read_data_dir("station_xml_epochs", "nc73631381_ad", "*.mseed")
    test_root = os.path.normpath(os.path.join(data_files[0], os.pardir))
    sc_ad = StreamCollection.from_directory(test_root)
    psc_ad = process_streams(sc_ad, event)
