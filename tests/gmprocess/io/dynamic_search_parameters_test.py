...
def test_dynamic_search_parameters(config):
    ...
    np.testing.assert_allclose(search_pars.duration, 6.5)

    magnitude = 6.0
    search_pars = SearchParameters(magnitude, conf, strec)
    np.testing.assert_allclose(search_pars.distance, 587.301345)
    np.testing.assert_allclose(search_pars.duration, 7.5)
    ...
    np.testing.assert_allclose(search_pars.duration, 8.5)
