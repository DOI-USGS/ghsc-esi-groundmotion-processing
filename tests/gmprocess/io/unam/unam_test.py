...
def test_unam():
    ...
    np.testing.assert_almost_equal(trace1.stats.coordinates.latitude, 18.567007)
    np.testing.assert_almost_equal(trace1.stats.coordinates.longitude, -101.977162)
...
def test_read_past_midnight():
    ...
    # record start time
    datafiles, event = read_data_dir("unam", "usp000cgtd", ["CUP50401.012"])
    stream = read_unam(datafiles[0])[0]
    trace = stream[0]
    assert trace.stats.starttime > event.time


def test_read_data():
    datafiles, _ = read_data_dir("unam", "us2000ar20", ["CANA1709.191", "PZPU1709.191"])
