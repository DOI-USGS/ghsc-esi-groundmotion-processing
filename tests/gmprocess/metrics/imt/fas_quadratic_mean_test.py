...
def test_fas():
    ...
    """
    fas_file = TEST_DATA_DIR / "fas_quadratic_mean.pkl"
    ...
    summary = StationSummary.from_stream(
        stream, ["quadratic_mean"], imts, bandwidth=30, config=config, event=event
    )

    pgms = summary.pgms
    for idx, f in enumerate(freqs):
        fstr = f"FAS({1 / f:.3f})"
        fval1 = pgms.loc[fstr, "QUADRATIC_MEAN"].Result
        fval2 = target_df.loc[fstr, "QUADRATIC_MEAN"].Result
