...
def test_waveform_metric():
    ...
    test_sa2 = waveform_metric_type.WaveformMetricType.metric_from_dict(test_sa_dict)
    assert test_sa2.__repr__() == "SA(T=1.0000, D=5.000): H1=0.710"

    test_pga = waveform_metric_type.PGA([0.14, 0.2], ["H1", "H2"])
    assert test_pga.values["H1"] == 0.14
    comps = test_pga.components
    assert len(comps) == 2
    assert "H1" in comps and "H2" in comps

    test_pgv = waveform_metric_type.PGV([0.23], ["H2"])
    assert test_pgv.__repr__() == "PGV: H2=0.230"

    test_duration = waveform_metric_type.Duration([3.24], ["H2"], "5-95")
