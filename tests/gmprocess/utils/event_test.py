...
def test_scalar_fromobspy():
    ...
    event = event_utils.ScalarEvent.from_obspy(obspy_event)
...
def test_scalar_fromparams():
    event = event_utils.ScalarEvent.from_params(**EVENT_INFO)
...
def test_scalar_fromworkspace():
    ...
    event = event_utils.ScalarEvent.from_workspace(filename)
    check_event(event, EVENT_INFO)


def test_scalar_from_geojson():
    filename = constants.TEST_DATA_DIR / "usp000hat0.geojson"
    event = event_utils.ScalarEvent.from_json(filename)
    check_event(event, EVENT_INFO)
...
def test_scalar_to_json():
    ...
    event_new = event_utils.ScalarEvent.from_json(scratch_dir / "event.json")
    event = event_utils.ScalarEvent.from_params(**EVENT_INFO)
...
def test_get_event_ids():
    # From list
    event_ids = event_utils.get_event_ids(ids=EVENT_IDS)
    assert EVENT_IDS == event_ids

    # from file
    filename = constants.TEST_DATA_DIR / "fdsn" / "event_ids.txt"
    event_ids = event_utils.get_event_ids(filename=filename)
    assert EVENT_IDS == event_ids

    # from directories
    data_dir = constants.TEST_DATA_DIR / "fdsn"
    event_ids = event_utils.get_event_ids(data_dir=data_dir)
    assert EVENT_IDS == event_ids

    # list, not file
    filename = constants.TEST_DATA_DIR / "fdsn" / "event_ids.txt"
    event_ids = event_utils.get_event_ids(ids=EVENT_IDS[0:5], filename=filename)
    assert EVENT_IDS[0:5] == event_ids

    # file, not directories
    filename = constants.TEST_DATA_DIR / "event_ids.txt"
    data_dir = constants.TEST_DATA_DIR / "fdsn"
    event_ids = event_utils.get_event_ids(filename=filename, data_dir=data_dir)
    assert EVENT_IDS[0:2] == event_ids


def test_events_from_workspaces():
    return
...
def test_parse_events_file():
    # File with event ids
    filename = constants.TEST_DATA_DIR / "fdsn" / "event_ids.txt"
    event_ids = event_utils.parse_events_file(filename=filename)
    assert EVENT_IDS == event_ids

    # File with event information
    EVENTS_DETAIL = [
        event_utils.ScalarEvent.from_params(
    ...
    events = event_utils.parse_events_file(filename=filename)
...
def test_write_event_geojson():
    ...
    scratch_dir = constants.TEST_DATA_DIR / "scratch"
    scratch_dir.mkdir(exist_ok=True)
    event_utils.write_event_geojson(data, scratch_dir)
    event_new = event_utils.ScalarEvent.from_json(scratch_dir / "event.json")
    event = event_utils.ScalarEvent.from_params(**EVENT_INFO)
    assert event_new == event
    shutil.rmtree(scratch_dir)
